...
export default function Equipes() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const res = await fetch('/api/n8n/webhook/user-team', {
          method: 'POST',
        ...
        const mapUser = (u) => ({
          id: u?.id ?? u?.user_id ?? u?.Usu�rio_id ?? null,
        ...
        const toTeam = (t) => ({
          ...
          membros: t?.membros ?? t?.users ?? t?.Usu�rios ?? [],
        })

        const collectUsers = () => {
          let arr = []
          if (Array.isArray(payload?.Usu�rios)) arr = payload.Usu�rios
        ...
        } else if (payload?.equipe || payload?.Usu�rios || payload?.users) {
          const base = toTeam(payload?.equipe || {})
          const members = base.membros?.length ? base.membros : (payload?.Usu�rios || payload?.users || [])
        ...
        if (!aborted) {
          ...
          setSupervisores(Usu�riosLista.filter(u => (u.role || '').toLowerCase().includes('supervisor')))
        }
      } catch (e) {
  ...
  async function handleAddTeamSubmit(event) {
    ...
    try {
      ...
      try {
        const payload = {
          id_Usu�rio: supervisorId, // enviar o ID do supervisor selecionado
          nome: nome,
        ...
        const resp = await fetch('/api/n8n/webhook/add-team', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        })
  ...
  async function handleAddUserSubmit(event) {
    ...
    try {
      const response = await fetch('/api/n8n/webhook/add-user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
