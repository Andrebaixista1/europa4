...
export async function importarCsvPrata({
  csv,
  nomeArquivo,
  empresa,
  tokenUsado,
  ...
  const resp = await fetch('/api/clientes-prata/import-csv', {
    method: 'POST',
...
export async function consultarImportacaoCsvPrataStatus({ jobId }) {
  const id = String(jobId ?? '').trim()
  if (!id) throw new Error('jobId obrigatório para consultar status.')

  const params = new URLSearchParams()
  params.set('jobId', id)
  const resp = await fetch(`/api/clientes-prata/import-status?${params.toString()}`, { method: 'GET' })
  const { data } = await parseJsonResponse(resp)
